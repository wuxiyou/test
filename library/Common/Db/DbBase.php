...
class DbBase
{
    ...
    {
        // [1] 参数判断。
        if (strlen($this->table_name) === 0 || !is_string($this->table_name)) {
            throw new \Exception(3002001, 'The table parameters is wrong');
        }
        if (!is_string($order)) {
            throw new \Exception(3001002, 'The order parameters is wrong');
        }
        if (!is_string($group)) {
            throw new \Exception(3001003, 'The group parameters is wrong');
        }
        if (!is_numeric($limit)) {
            throw new \Exception(3001004, 'The limit parameter is wrong');
        }
        // [2] where 条件生成。
        ...
        if ($limit == 0) {
            $sql = "SELECT {$column_condition} FROM `{$this->table_name}` WHERE {$where_condition} {$group_by} {$order_by}";
        } else {
            $sql = "SELECT {$column_condition} FROM `{$this->table_name}` WHERE {$where_condition} {$group_by} {$order_by} LIMIT {$limit}";
        }
    ...
    public function fetchOne(array $data, array $where, $order = '', $group = '')
    {
        ...
        if (empty($where)) {
            throw new \Exception(3001002, 'The where parameters is wrong');
        }
        if (!is_string($order)) {
            throw new \Exception(3001003, 'The order parameters is wrong');
        }
        if (!is_string($group)) {
            throw new \Exception(3001004, 'The group parameters is wrong');
        }
        // [2] where 条件生成。
        $where_condition = ' 1 = 1 ';
        $params = [];
        ...
        $sql = "SELECT {$column_condition} FROM `{$this->table_name}` WHERE {$where_condition} {$group_by} {$order_by} LIMIT 1";
        $sth = $this->link->prepare($sql);
        $sth->execute($params);
    ...
    public function count(array $where)
    {
        ...
        $sql = "SELECT {$column_condition} FROM `{$this->table_name}` WHERE {$where_condition} LIMIT 1";
        $sth = $this->link->prepare($sql);
        $sth->execute($params);
        $data = $sth->fetch(\PDO::FETCH_ASSOC);
    ...
    public function insert(array $data)
    {
        ...
        if (empty($data)) {
            throw new \Exception(3002002, 'The data parameter can\'t be empty');
        }
        $column_condition = '';
        $column_question = '';
        $params = [];
        ...
        $sql = "INSERT INTO `{$this->table_name}` ($column_condition) VALUES($column_question) ";
        $sth = $this->link->prepare($sql);
    ...
    public function update(array $data, array $where)
    {
        ...
        if (empty($where)) {
            throw new \Exception(3001001, 'The where parameters is wrong');
        }
        if (empty($data)) {
            throw new \Exception(3001004, 'The data parameter cannot be empty');
        }
        // [2] SET 条件生成。
        $set_condition = '';
        $params = [];
        ...
        $sql = "UPDATE `{$this->table_name}` SET {$set_condition} WHERE {$where_condition} ";
        $sth = $this->link->prepare($sql);
        $ok = $sth->execute($params);
    ...
    {
        if (empty($params)) {
            throw new \Exception(3001101, 'where in params 有误');
        }
        $question = '';
        $values = [];
        $index = 0;
    ...
    {
        if (empty($this->stmt)) {
            throw new \Exception(-1, '请正确使用rawFetchOne()方法');
        }
    ...
    {
        if (empty($this->stmt)) {
            throw new \Exception(-1, '请正确使用rawFetchAll()方法');
        }
